...
## 2. Fix Email Confirmation (Run this if you get "Email not confirmed")

```sql
-- Confirm your user manually
UPDATE auth.users 
SET email_confirmed_at = NOW(), 
    confirmed_at = NOW(),
    raw_user_meta_data = raw_user_meta_data || '{"email_verified": true}'::jsonb
WHERE email = 'jacknishad1111@gmail.com';
```
...
```sql
-- Check user status
SELECT id, email, confirmed_at FROM auth.users WHERE email = 'jacknishad1111@gmail.com';
```
...
3. You should see the MoneyTrail dashboard!
